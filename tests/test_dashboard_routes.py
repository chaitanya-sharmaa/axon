...
from axon.api.routes.dashboard_routes import router
...
def test_get_requests(auth_headers):
    with patch("axon.api.routes.dashboard_routes.request_logger") as mock_logger:
...
async def test_get_cache_entries(auth_headers):
    with patch("axon.api.routes.dashboard_routes.semantic_cache") as mock_cache:
...
def test_get_events(auth_headers):
    with patch("axon.api.routes.dashboard_routes.event_logger") as mock_events:
...
def test_get_tenants_and_sessions(auth_headers):
    with patch("axon.api.routes.dashboard_routes.memory_store") as mock_store:
...
def test_get_tenants_and_sessions_no_support(auth_headers):
    with patch("axon.api.routes.dashboard_routes.memory_store", spec=[]):
        res_t = client.get("/admin/tenants", headers=auth_headers)
        assert res_t.json() == []
...
def test_get_agentic_stats(auth_headers):
    with patch("axon.api.routes.dashboard_routes.request_logger") as mock_logger:
        ...
        with patch("axon.api.routes.dashboard_routes.agentic_state_manager") as mock_manager:
